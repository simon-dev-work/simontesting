...
const BookingPage = () => {
  ...
  const [practiceInfo, setPracticeInfo] = useState({ name: '', email: '', shortName: '' });
  ...
  const [apiPracticeDetails, setApiPracticeDetails] = useState({ name: '', email: '', shortName: '' });
  const [isPortal, setIsPortal] = useState(true);

  // Use practice_id from URL params as primary source, fallback to siteSettings
  const practiceId = params?.practice_id || siteSettings?.practiceId;
  ...
  useEffect(() => {
    ...
    const fetchPractitioners = async () => {
      try {
        ...
        // Store practice details from API
        if (response.data) {
          setPracticeInfo({
            name: response.data.name || '',
            email: response.data.email || '',
            shortName: response.data.short_name || response.data.name || ''
          });
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'date' && value && !formData.practitioner && actionVerb === 'book') {
  ...
  const handleRequestCall = async () => {
    ...
    try {
      ...
      formDataToSend.append('practice[name]', apiPracticeDetails.name || practiceInfo.name || 'Practice');
      formDataToSend.append('practice[email]', apiPracticeDetails.email || practiceInfo.email || 'support@nevadacloud.com');
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: primaryColor }}>
      {/* Navbar */}
      {isPortal ? (
        <SinglePageNavbar
          practiceId={practiceId}
          logoLight={siteSettings?.about?.logo_light}
          logoDark={siteSettings?.about?.logo_dark}
        />
      ) : (
        <Navbar
          logoLight={siteSettings?.about?.logo_light}
      ...
      <div className="flex-grow relative">
        ...
        <div className="w-full h-[500px] bg-[url('/images/InfoCentre/InfoCentreBannerGeneric.jpg')] bg-cover bg-center text-white">
          <div className="h-full bg-black bg-opacity-50 flex items-center justify-center">
            <h1 className="text-5xl font-bold text-center px-4">{siteSettings?.name || 'Practice'} Online Booking Page</h1>
          </div>
        </div>

        {/* Background Image for booking section */}
                    ...
                    <div>
                      ...
                      <ul className="space-y-1">
                        <li className="text-gray-600 flex items-center gap-2">
                          ...
                          <a href={`tel:${siteSettings.tel}`} className="text-gray-600 hover:text-primary transition-colors">
                            {siteSettings.tel}
                          </a>
                        </li>
                        <li className="text-gray-600 flex items-center gap-2">
                          <FaEnvelope className="text-primary" />
                          <a href={`mailto:${siteSettings.email}`} className="text-gray-600 hover:text-primary transition-colors">
                            {siteSettings.email}
                          </a>
                        </li>
